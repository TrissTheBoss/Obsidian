...
## D-0023 - Keep the first graphics path on public Blaze3D

**Status:** ACTIVE  
**Decision:** Obsidian's initial graphics-pipeline/render-pass implementation will stay on Minecraft 26.2's public Blaze3D API. Native Vulkan/backend access is not introduced merely because Obsidian is Vulkan-only; it remains an evidence-driven escalation path for capabilities or performance controls the public layer demonstrably cannot provide.  
**Why:** Exact dev6 inspection proved the public layer exposes offscreen texture/view creation, explicit graphics pipeline construction and precompilation, vertex/index buffers, render passes, indexed draws, texture-to-buffer readback, command submission, and timestamps. Backend inspection further confirmed `precompilePipeline(pipeline, customShaderSource)` and `RenderPass.setPipeline(pipeline)` share the Vulkan device's identity-keyed pipeline cache, so custom Obsidian shader source can be compiled once and reused without a native hook.  
**Effect:** Dev6's first draw uses a private Blaze3D RGBA8 target and public render-pass APIs, preserving Minecraft's device/swapchain ownership. Future native Vulkan access requires a concrete missing feature or measured performance problem and a new decision documenting the ownership/synchronization tradeoff.
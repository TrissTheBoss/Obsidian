...
/**
 * P2.5 live comparison for exact vanilla-emitted arbitrary MODEL quads grouped
 * into SOLID and CUTOUT public BLOCK-format indexed-indirect passes.
 */
public final class RealSectionBroadModelProbe implements AutoCloseable {
    private static final System.Logger LOG = System.getLogger("Obsidian/RealSectionBroadModelProbe");
    private static final Supplier<String> SOLID_PASS_LABEL = () -> "Obsidian Phase 2 dev5 SOLID comparison";
    private static final Supplier<String> CUTOUT_PASS_LABEL = () -> "Obsidian Phase 2 dev5 CUTOUT comparison";
    private static final Supplier<String> INDIRECT_LABEL = () -> "Obsidian Phase 2 dev5 layered indirect commands";
    ...
    public RealSectionBroadModelProbe(
            GpuDevice device,
        ...
        if (!device.getDeviceInfo().features().drawIndirect()) {
            throw new IllegalStateException("Phase 2 dev5 requires indexed indirect drawing");
        }
    }
    ...
    public void afterWorldRender(GameRenderer renderer, long frameSerial) {
        ...
        try {
            ...
            LOG.log(System.Logger.Level.ERROR,
                    "Phase 2 dev5 layered comparison draw failed; beginning completion-gated cleanup.", e);
    ...
    private void tryCaptureBuildUploadAndDraw(GameRenderer renderer, long frameSerial, long nowNs) {
        ...
        try {
            ...
            if (firstReference.faceCount() <= 0 || !firstReference.contentEquals(secondReference)) {
                throw new IllegalStateException("Phase 2 dev5 permanent cube oracle is empty or nondeterministic");
            }
            ...
            if (!firstBaked.contentEquals(secondBaked)) {
                throw new IllegalStateException("Phase 2 dev5 generalized vanilla quad capture is nondeterministic");
            }
            if (firstBaked.solidQuads() <= 0 || firstBaked.cutoutQuads() <= 0) {
                if (!waitingLayerLogged) {
                    waitingLayerLogged = true;
                    LOG.log(System.Logger.Level.INFO,
                            "Phase 2 dev5 needs a section containing both supported SOLID and CUTOUT MODEL quads for the combined validation gate. Current section=({0},{1},{2}), solidQuads={3}, cutoutQuads={4}; move near ordinary terrain plus grass/flowers or other cutout blocks.",
            ...
            if (!firstDrawable.contentEquals(secondDrawable)) {
                throw new IllegalStateException("Phase 2 dev5 layered BLOCK mesh is nondeterministic");
            }
            if (firstDrawable.quadCount() != firstBaked.quadCount()
                ...
                throw new IllegalStateException("Phase 2 dev5 generalized capture/drawable accounting mismatch");
            }
            if (firstDrawable.vertexBytes() + firstDrawable.indexBytes() > BakedSectionMesh.MAX_UPLOAD_BYTES) {
                throw new IllegalStateException("Phase 2 dev5 mesh exceeded its bounded upload contract");
            }

            vertexHandle = arena.allocate(firstDrawable.vertexBytes(), 16);
            if (vertexHandle == DeviceGeometryArena.INVALID_HANDLE) {
                throw new IllegalStateException("Device arena could not fit Phase 2 dev5 BLOCK vertex data");
            }
            indexHandle = arena.allocate(firstDrawable.indexBytes(), 4);
            if (indexHandle == DeviceGeometryArena.INVALID_HANDLE) {
                throw new IllegalStateException("Device arena could not fit Phase 2 dev5 index data");
            }
            ...
            if (!staging.beginBatch()) {
                throw new IllegalStateException("Phase 2 dev5 could not open bounded staging batch");
            }
            batchOpen = true;
            ...
            if (!staging.stageCopy(encoder, firstDrawable.vertexBuffer(), vertexSlice)
                ...
                throw new IllegalStateException("Phase 2 dev5 layered upload hit bounded staging backpressure");
            }
            ...
            LOG.log(System.Logger.Level.ERROR,
                    "Phase 2 dev5 generalized section setup failed; Minecraft will continue for diagnosis.", e);
        }
    }
    ...
    private void ensurePipelines() {
        ...
        solidPipeline = buildComparisonPipeline(
                "obsidian_phase2_dev5_solid", RenderPipelines.SOLID_BLOCK, depthTemplate, false);
        cutoutPipeline = buildComparisonPipeline(
                "obsidian_phase2_dev5_cutout", RenderPipelines.CUTOUT_BLOCK, depthTemplate, true);
        ...
        if (!pipelineValid) {
            throw new IllegalStateException("Phase 2 dev5 public SOLID/CUTOUT BLOCK comparison pipelines failed to compile");
        }
    }
    ...
    private void submitDraw(GameRenderer renderer, long frameSerial, boolean finalDraw) {
        ...
        if (finalDraw) {
            ...
            LOG.log(System.Logger.Level.INFO,
                    "Phase 2 dev5 layered comparison window finished on frame {0}: comparisonDraws={1}, comparisonNs={2}, usefulSubmissions={3}, resourceEpochChecks={4}, profilerOnlySubmissions=0, indirectCalls={5}, trianglesSubmitted={6}; waiting nonblockingly for completion-gated reclamation.",
    ...
    private void encodeLiveDraw(CommandEncoder encoder, GameRenderer renderer) {
        if (drawableMesh == null || bakedSnapshot == null || indirectCommands == null) {
            throw new IllegalStateException("Phase 2 dev5 generalized resources are incomplete");
        }
        long currentEpoch = SectionMaterialSnapshot.currentResourceEpoch();
        resourceEpochChecks++;
        if (currentEpoch != bakedSnapshot.resourceEpoch()) {
            throw new IllegalStateException("Minecraft model/atlas resource epoch changed during P2.5 comparison");
        }
        ...
        if (!Double.isFinite(relativeX) || !Double.isFinite(relativeY) || !Double.isFinite(relativeZ)) {
            throw new IllegalStateException("Non-finite Phase 2 dev5 camera-relative section origin");
        }
        ...
        if (dynamicTransforms == null || projection == null || fog == null || RenderSystem.getGlobalSettingsUniform() == null) {
            throw new IllegalStateException("Minecraft world uniform buffers are unavailable for P2.5");
        }
    ...
    private void beginCleanupRetirement(long frameSerial) {
        ...
        try {
            ...
            LOG.log(System.Logger.Level.ERROR,
                    "Phase 2 dev5 could not submit completion-gated failure cleanup; preserving resources for bounded shutdown/device teardown.",
                    cleanupFailure);
            state = State.FAILED;
        }
    }
    ...
    private boolean tryRegisterRetirements(long frameSerial) {
        if (pendingArenaFence != null) {
            try {
                ...
                LOG.log(System.Logger.Level.ERROR,
                        "Phase 2 dev5 arena retirement registration failed; preserving completion handle for retry.", e);
                return false;
            }
        }
        if (pendingResourceFence != null && indirectCommands != null) {
            try {
                ...
                retirementRegistrationFailures++;
                LOG.log(System.Logger.Level.ERROR,
                        "Phase 2 dev5 indirect-command retirement registration failed; preserving completion handle for retry.", e);
                return false;
            }
        }
    ...
    private int checkedFirstIndex(GpuBufferSlice indexSlice) {
        if ((indexSlice.offset() & (Integer.BYTES - 1L)) != 0L) {
            throw new IllegalStateException("Phase 2 dev5 index allocation is not 4-byte aligned");
        }
        long value = indexSlice.offset() / Integer.BYTES;
        if (value > Integer.MAX_VALUE) {
            throw new IllegalStateException("Phase 2 dev5 firstIndex exceeds public indirect command range");
        }
        return (int) value;
    }
    ...
    private static ByteBuffer indirectCommand(int indexCount, int firstIndex) {
        if (indexCount <= 0) {
            throw new IllegalArgumentException("P2.5 indirect layer index count must be positive");
        }
    ...
    private void closeIndirectIfOwned() {
        ...
        try {
            ...
            LOG.log(System.Logger.Level.WARNING, "Failed to close unsubmitted dev5 indirect command buffer.", e);
        } finally {
            indirectCommands = null;
        }
    }

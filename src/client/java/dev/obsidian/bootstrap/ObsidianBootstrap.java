...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev21 P3.9 caller-specific single-section tracer armed. The validated production renderer, permanent P3.7 differential oracle, dev17 corrected shadow proof, dev18 pending same-section coalescing semantics and dev20 outer section-dirty origin tracer remain unchanged. Dev21 is diagnostic-only: it distinguishes exact Minecraft 26.2 ClientChunkCache.onLightUpdate single-section invalidations from ClientPacketListener.handleChunksBiomes broad invalidations and residual other single-section calls, then correlates that bounded caller evidence with the already-existing provenance fallback. Missing provenance still falls back exactly as before. Production full-section capture/mesh/upload/install/draw remains authoritative and partial GPU patching is disabled. Frozen A-0159 thresholds, four fixed slices, ClientLevel.setBlocksDirty provenance source, admission policy, greedy eligibility, shaders, pipelines, atlas/lightmap semantics, worker count/backpressure and resource lifetime remain unchanged; partialRemeshGpuInstallChanged=false, partialRemeshRenderedGeometryChanged=false, workerWorldReadsAfterCapture=0.");
    }

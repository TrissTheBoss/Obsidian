...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev4 P3.2 binary visibility validation armed. The existing renderer-owned 3x3 async scene and relevance-aware bounded worker scheduler remain active. Every production worker job now derives a six-direction 4096-bit visibility sidecar from the immutable SectionSnapshot using reusable 18x18 primitive row scratch, while periodic worker audits prove deterministic mask construction and exact set equivalence against the independent ReferenceFaceMesh oracle. Live world/model/material/light capture and all GPU allocation/upload/draw/retirement remain render-thread-only. The generalized SOLID/CUTOUT BakedSectionMesh remains the production drawable output; binaryVisibilitySidecarIntegrated=true, greedyRectangleEmission=false, workerWorldReadsAfterCapture=0. P3.3 greedy rectangle extraction is still downstream.");
    }

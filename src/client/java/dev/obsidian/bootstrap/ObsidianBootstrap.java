...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev2 async scene integration armed. The persistent renderer-owned 3x3 section scene keeps exact render-thread SectionSnapshot plus generalized vanilla model/material/light capture, then submits only immutable inputs to bounded priority mesh workers. Center work is HIGH priority and neighboring records are NORMAL priority. Completed worker meshes are generation/lifecycle/resource-epoch checked before any render-thread GPU allocation, bounded staging upload, indexed-indirect install or completion-gated retirement. Safe cancelled/stale pre-install work is discarded without GPU ownership. synchronousSceneMeshBuilds=0, productionSceneInstallStillSynchronous=false, productionWorkerSceneIntegration=true. Vanilla terrain remains active; greedy/bitmask meshing and worker-local scratch optimization remain later Phase 3 work.");
    }

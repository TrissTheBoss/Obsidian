...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev11 P3.4 repeat-aware greedy GPU emission canary armed. The proven P3.2 visibility masks, P3.3 topology rectangles, dev6 canonical render keys, dev7 render-key-aware candidates, dev8 ordinary emission-safety classifier, dev9 repeat-aware UV descriptors and dev10 transport proof remain active. Each production worker now also builds a deterministic hybrid RepeatAwareGreedyMesh: only exact dev10 transport-safe canonical groups are replaced by one large quad, while every unsafe, ambiguous, noncanonical or generalized baked quad remains exact passthrough. The merged path preserves source vertex order/diagonal, repeats in candidate-local sprite coordinates, remaps through the exact raw atlas affine basis, uses explicit textureGrad gradients derived from the unwrapped repeat coordinate, and binds the same live blocks-atlas view/sampler plus live lightmap as the current block path. Four fixed public indexed-indirect classes are used: passthrough SOLID, passthrough CUTOUT, merged SOLID and merged CUTOUT. Full-atlas sampler wrapping remains forbidden. Because dev11 changes emitted geometry, repeat-reset lines and rectangle/section T-junctions are now an explicit runtime visual/raster validation obligation: automated evidence alone cannot promote dev11 and an explicit human visual PASS is mandatory. All live world/model/material/light capture and GPU allocation/upload/draw/retirement remain render-thread-only; binaryVisibilitySidecarIntegrated=true, greedyRectangleSidecarIntegrated=true, renderMergeKeySidecarIntegrated=true, renderMergeCandidateSidecarIntegrated=true, ordinaryQuadEmissionSafetySidecarIntegrated=true, repeatAwareUvDescriptorSidecarIntegrated=true, repeatAwareTransportSidecarIntegrated=true, repeatAwareGreedyMeshIntegrated=true, repeatAwareGreedyGpuEmission=true, renderCorrectMergeKeyComplete=false, workerWorldReadsAfterCapture=0.");
    }

...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev8 P3.4 ordinary four-vertex emission-safety validation armed. The proven P3.2 visibility masks, P3.3 topology rectangles, dev6 canonical render keys and dev7 render-key-aware candidates remain active. Every production worker now also classifies each dev7 candidate against exact repeated-field continuity for per-corner ARGB color, packed light and raw atlas UV bits. A multi-face candidate is ordinary-attribute-safe only when all three fields can be represented exactly by one ordinary four-corner rectangle; a measured safe count of zero is valid evidence and indicates a repeat-aware representation is required. Arbitrary generalized baked geometry remains on the exact BakedSectionMesh path. Live world/model/material/light capture and all GPU allocation/upload/draw/retirement remain render-thread-only; binaryVisibilitySidecarIntegrated=true, greedyRectangleSidecarIntegrated=true, renderMergeKeySidecarIntegrated=true, renderMergeCandidateSidecarIntegrated=true, ordinaryQuadEmissionSafetySidecarIntegrated=true, greedyRectangleGpuEmission=false, renderCorrectMergeKeyComplete=false, workerWorldReadsAfterCapture=0.");
    }

...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev13 P3.6 T-junction policy evidence armed. The fully validated P3.5 border/halo path and dev11 repeat-aware greedy GPU emission remain unchanged. Dev13 adds only a worker-side immutable topology proof over the actual dev10-safe merged candidates and runtime evidence that junction-bearing LIVE sections use the existing camera-relative section transform before float conversion. Every completed worker ticket builds the T-junction proof twice before publication; installed evidence requires exact integer plane/lattice/bounds identities and real strict T-junctions. No candidate eligibility, source suppression, vertex position, vertex/index format, shader, pipeline, atlas/lightmap, draw-class or native graphics semantics change. The targeted visual gate arms only after automated evidence proves strict T-junctions are actually present in a drawn LIVE section. All live world/model/material/light capture and GPU allocation/upload/draw/retirement remain render-thread-only; tJunctionTopologyProofIntegrated=true, repeatAwareGreedyGpuEmission=true, tJunctionGeometryChanged=false, tJunctionShaderChanged=false, tJunctionPipelineChanged=false, workerWorldReadsAfterCapture=0.");
    }

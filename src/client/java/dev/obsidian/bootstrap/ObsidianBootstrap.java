...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev20 P3.9 section-dirty origin tracer armed. The validated production renderer, permanent P3.7 differential oracle, dev17 corrected shadow proof and dev18 pending same-section coalescing semantics remain unchanged. Dev20 is diagnostic-only: it classifies tracked-scene-relevant LevelExtractor section-dirty calls by exact-block, block-range, neighbor-range, section-range, single-section or unclassified origin so A-0171 missing/empty provenance fallbacks can be traced without changing the fallback decision. Production full-section capture/mesh/upload/install/draw remains authoritative and partial GPU patching is disabled. Frozen A-0159 thresholds, four fixed slices, ClientLevel.setBlocksDirty provenance source, admission policy, greedy eligibility, shaders, pipelines, atlas/lightmap semantics, worker count/backpressure and resource lifetime remain unchanged; partialRemeshGpuInstallChanged=false, partialRemeshRenderedGeometryChanged=false, workerWorldReadsAfterCapture=0.");
    }

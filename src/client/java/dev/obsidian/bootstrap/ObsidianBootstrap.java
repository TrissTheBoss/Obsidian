...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev6 P3.4 render-correct merge-key sidecar validation armed. The proven P3.2 binary visibility masks and P3.3 topology rectangles remain active. Every production worker job now also maps conservative canonical faces to exact vanilla-baked SOLID/CUTOUT quads only when full unit-face geometry is uniquely proven. Render equivalence requires exact direction, layer, material/sprite/tint/shade/emission/animation identity, corner-order signature, raw UV bits, exact ARGB color and packed light. Offset, partial, ambiguous and otherwise generalized model geometry remains on the exact BakedSectionMesh passthrough path. Live world/model/material/light capture and all GPU allocation/upload/draw/retirement remain render-thread-only; binaryVisibilitySidecarIntegrated=true, greedyRectangleSidecarIntegrated=true, renderMergeKeySidecarIntegrated=true, greedyRectangleGpuEmission=false, renderCorrectMergeKeyComplete=false, workerWorldReadsAfterCapture=0.");
    }

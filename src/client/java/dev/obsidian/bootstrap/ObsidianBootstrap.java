...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev3 scheduler/backpressure validation armed. The persistent renderer-owned 3x3 scene keeps exact render-thread snapshot/model/material/light capture and submits immutable mesh inputs to globally relevance-aware bounded workers: center HIGH, cardinal neighbors NORMAL, diagonal neighbors LOW. Scene admission is capped at two new records per render frame with explicit backpressure; workers reuse primitive build scratch and perform periodic full determinism audits. Completed meshes remain generation/lifecycle/resource-epoch checked before render-thread GPU allocation, bounded staging, indexed-indirect install and completion-gated retirement. A diagnostic-only fixed lifecycle anchor counts first-scene chunk unload/return events without driving active scene invalidation. synchronousSceneMeshBuilds=0, productionSceneInstallStillSynchronous=false, productionWorkerSceneIntegration=true. Vanilla terrain remains active; greedy/bitmask meshing remains downstream P3.2 work.");
    }

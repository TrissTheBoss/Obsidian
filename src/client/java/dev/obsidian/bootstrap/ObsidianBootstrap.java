...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev9 P3.4 repeat-aware UV descriptor validation armed. The proven P3.2 visibility masks, P3.3 topology rectangles, dev6 canonical render keys, dev7 render-key-aware candidates and dev8 ordinary emission-safety classifier remain active. Every production worker now additionally proves whether each dev7 multi-face candidate has an exact two-U by two-V source atlas rectangle with an affine geometric-corner-to-UV-corner orientation, retaining raw atlas bounds and orientation for sprite-local repetition evidence. Repeat happens in sprite-local space before remapping into the source atlas rectangle; full-atlas sampler wrapping is not the correctness model. Repeat-aware four-vertex feasibility additionally requires dev8 color and light interpolation safety. Zero representable or safe candidates is valid evidence. Arbitrary generalized baked geometry remains on the exact BakedSectionMesh path. Live world/model/material/light capture and all GPU allocation/upload/draw/retirement remain render-thread-only; binaryVisibilitySidecarIntegrated=true, greedyRectangleSidecarIntegrated=true, renderMergeKeySidecarIntegrated=true, renderMergeCandidateSidecarIntegrated=true, ordinaryQuadEmissionSafetySidecarIntegrated=true, repeatAwareUvDescriptorSidecarIntegrated=true, greedyRectangleGpuEmission=false, renderCorrectMergeKeyComplete=false, workerWorldReadsAfterCapture=0.");
    }

...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev12 P3.5 border/halo correctness armed. The fully validated dev11 repeat-aware greedy GPU emission canary remains unchanged: exact dev10-safe canonical groups use the repeat-aware large-quad path and unsupported/ambiguous/unsafe/generalized geometry remains exact passthrough. Dev12 changes no geometry, vertex/index format, shader, pipeline or native graphics behavior. It closes the next correctness boundary by expanding section-dirty relevance to the conservative 5x3x5 section dependency volume implied by the existing one-block snapshot halo, validating all 1,536 outward boundary cells of every installed record against both BinarySectionVisibility and the independent ReferenceFaceMesh, fingerprinting exact vanilla-captured border light/color payload, and checking independently captured LIVE neighbor snapshots across every shared X/Z border. Existing generation-safe stale-result rejection applies to newly relevant halo-only dirty events. All live world/model/material/light capture and GPU allocation/upload/draw/retirement remain render-thread-only; borderHaloProofIntegrated=true, repeatAwareGreedyGpuEmission=true, borderHaloGeometryChanged=false, renderCorrectMergeKeyComplete=false, workerWorldReadsAfterCapture=0. P3.6 broader T-junction policy remains later work.");
    }

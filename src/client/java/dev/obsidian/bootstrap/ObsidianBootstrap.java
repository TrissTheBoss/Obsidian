...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev22 P3.9 exact-section light-update preservation correction armed. A-0181 proved the observed missing/empty provenance fallback population was 43/43 outer SINGLE_SECTION and 43/43 LIGHT_UPDATE-only with zero biome/other/mixed/unavailable, cross-thread or overflow evidence. Dev22 changes only the shadow partial-remesh admission exception: an already-pending exact episode may survive an otherwise-empty provenance drain only when every accepted lifecycle event is a same-section light update for exactly that pending section. Any mismatch fails closed to the existing provenance fallback. Production full-section invalidation/capture/mesh/upload/install/draw remains authoritative and unchanged; partial GPU patching remains disabled. Frozen A-0159 thresholds, four fixed slices, exact block-local provenance source, permanent P3.7 oracle, greedy eligibility, shaders, pipelines, atlas/lightmap semantics, worker count/backpressure and resource lifetime remain unchanged; partialRemeshGpuInstallChanged=false, partialRemeshRenderedGeometryChanged=false, workerWorldReadsAfterCapture=0.");
    }

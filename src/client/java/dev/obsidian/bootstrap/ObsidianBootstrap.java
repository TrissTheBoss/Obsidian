...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev10 P3.4 repeat-aware transport/sampling proof armed. The proven P3.2 visibility masks, P3.3 topology rectangles, dev6 canonical render keys, dev7 render-key-aware candidates, dev8 ordinary emission-safety classifier and dev9 repeat-aware UV descriptors remain active. Every production worker now additionally proves the no-emission transport contract for each dev9 repeat-aware-four-vertex-safe multi-face candidate: preserve source baked vertex order/diagonal, repeat in candidate-local sprite coordinates, remap through the exact dev9 raw atlas rectangle/orientation, require explicit gradients derived from the unwrapped repeat coordinate, require the same live blocks-atlas view and sampler as the existing block path, and retain the positive outer-edge endpoint policy. Internal repeat-line raster ownership remains an explicit later visual/raster obligation rather than being falsely claimed solved; no greedy GPU geometry is emitted in dev10. Arbitrary generalized baked geometry remains on the exact BakedSectionMesh path. Live world/model/material/light capture and all GPU allocation/upload/draw/retirement remain render-thread-only; binaryVisibilitySidecarIntegrated=true, greedyRectangleSidecarIntegrated=true, renderMergeKeySidecarIntegrated=true, renderMergeCandidateSidecarIntegrated=true, ordinaryQuadEmissionSafetySidecarIntegrated=true, repeatAwareUvDescriptorSidecarIntegrated=true, repeatAwareTransportSidecarIntegrated=true, greedyRectangleGpuEmission=false, renderCorrectMergeKeyComplete=false, workerWorldReadsAfterCapture=0.");
    }

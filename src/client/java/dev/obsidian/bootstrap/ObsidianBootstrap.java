...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev7 P3.4 render-key-aware merge-candidate sidecar validation armed. The proven P3.2 binary visibility masks, P3.3 topology rectangles and dev6 canonical render keys remain active. Every production worker job now additionally partitions only uniquely mapped canonical faces into deterministic same-render-key rectangle candidates with exact no-gap/no-overlap eligible-face coverage, bounded primitive scratch and deterministic audits. Ambiguous/unmapped canonical faces and arbitrary generalized baked geometry remain on the exact BakedSectionMesh passthrough path. Dev7 does not claim rectangle-level interpolation or atlas-UV emission safety; live world/model/material/light capture and all GPU allocation/upload/draw/retirement remain render-thread-only; binaryVisibilitySidecarIntegrated=true, greedyRectangleSidecarIntegrated=true, renderMergeKeySidecarIntegrated=true, renderMergeCandidateSidecarIntegrated=true, greedyRectangleGpuEmission=false, renderCorrectMergeKeyComplete=false, workerWorldReadsAfterCapture=0.");
    }

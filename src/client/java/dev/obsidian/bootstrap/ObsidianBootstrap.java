...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev14 P3.7 differential correctness evidence armed. The fully validated P3.6 T-junction policy, P3.5 border/halo path and dev11 repeat-aware greedy GPU emission remain unchanged. Dev14 adds only a worker-side immutable differential proof that expands the final retained optimized identities back to permanent independent ReferenceFaceMesh topology and frozen SectionBakedQuadSnapshot render truth. Every completed worker ticket builds the differential proof twice before publication; real mismatches fail before install and retain a deterministic first-failure fixture. No candidate eligibility, source suppression, vertex position, vertex/index format, shader, pipeline, atlas/lightmap, draw-class or native graphics semantics change. All live world/model/material/light capture and GPU allocation/upload/draw/retirement remain render-thread-only; differentialCorrectnessProofIntegrated=true, tJunctionTopologyProofIntegrated=true, repeatAwareGreedyGpuEmission=true, differentialGeometryChanged=false, differentialShaderChanged=false, differentialPipelineChanged=false, workerWorldReadsAfterCapture=0.");
    }

...
public final class ObsidianBootstrap {
    ...
    public static void onMinecraftReady() {
        ...
        LOG.log(System.Logger.Level.INFO,
                "Obsidian Phase 3 dev19 P3.9 provenance diagnostic armed. The validated production renderer, permanent P3.7 differential oracle, dev17 corrected shadow proof and dev18 pending same-section coalescing semantics remain unchanged. Dev19 adds only bounded observational diagnostics around the existing provenance fallback so missing/empty dirty provenance, off-render-thread provenance and bounded-bridge overflow can be distinguished. Production full-section capture/mesh/upload/install/draw remains authoritative and partial GPU patching is disabled. Frozen A-0159 thresholds, four fixed slices, ClientLevel.setBlocksDirty provenance surface, admission policy, greedy eligibility, shaders, pipelines, atlas/lightmap semantics, worker count/backpressure and resource lifetime remain unchanged; partialRemeshGpuInstallChanged=false, partialRemeshRenderedGeometryChanged=false, workerWorldReadsAfterCapture=0.");
    }
